...
class ModifyBufferStreamFilter : public Http::PassThroughFilter, public Logger::Loggable<Logger::Id::filter> {
public:
  Http::FilterDataStatus decodeData(Buffer::Instance& data, bool end_stream) {
    ENVOY_LOG(info, "\n################INSIDE DECODEDATA OF NEW FILTER! end_stream is {}", end_stream);
  ...
  Http::FilterDataStatus encodeData(Buffer::Instance& data, bool end_stream) {
    ENVOY_LOG(info, "\n****************INSIDE ENCODEDATA OF NEW FILTER! end_stream is {}", end_stream);
